...
# 冷却配置
cooldown:
  default: 0                   # 默认冷却时间（tick）
  cleanup_interval: 6000       # 冷却数据清理间隔（tick，5分钟）

# ModelEngine集成配置
modelengine:
  enabled: true                # 是否启用ModelEngine集成
  cache_expire: 12000          # 模型缓存过期时间（tick，10分钟）

# MythicMobs集成配置
mythicmobs:
  timeout: 1000               # 技能执行超时时间（毫秒）
  retry_attempts: 3           # 失败重试次数

# 性能监控配置
metrics:
  enabled: true               # 是否启用性能指标收集
  log_interval: 72000         # 性能日志输出间隔（tick，1小时）
```
