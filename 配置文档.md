...
| `swing` | 挥手动作 | PlayerAnimationEvent | 玩家左键主手挥动时 |
...
meta:
  ...
  # 是否主动取消原始事件（仅对可取消事件生效：attack/damaged 的 when=instant）
  # 说明：当通过 API 调用 MythicMobs 技能时，其内部 cancelevent 可能无法关联到 Bukkit 原事件。
  # 将该开关置为 true 后，插件会在对应监听中主动 setCancelled(true)。
...
同时为了确保在 API 触发场景也能稳定生效，建议在对应规则中开启 `meta.cancel_event: true`，
插件将主动取消 Bukkit 原事件，避免出现因上下文缺失导致的取消失败。
...
- id: magic_attack
  ...
  target: @victim
  meta:
    cancel_event: true   # 确保在 API 触发场景也能取消原始事件
...
- 插件使用 `EventPriority.HIGH` 确保技能能够取消事件
 - 若仅在 MythicMobs 技能中使用 `cancelevent` 而未在规则中启用 `meta.cancel_event: true`，
   在 API castSkill 触发的场景下可能不会取消原事件；建议两者配合使用以确保一致性。
